...
struct AddressHasher {
  ...
  size_t operator()(const Address& adr) const {
    size_t a = shash(adr.city);
    size_t b = shash(adr.street);
    size_t c = ihash(adr.building);
    return a*h*h + b*h + c;
  }
...
struct PersonHasher {
  ...
  hash<int> ihash;
  hash<double> dhash;
  AddressHasher Adr_hash;
  size_t h = 7;
  size_t operator()(const Person& p) const {
    size_t a = shash(p.name);
    size_t b = Adr_hash(p.address);
    size_t c = ihash(p.height);
    size_t d = dhash(p.weight);
