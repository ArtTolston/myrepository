...
public:
	...
	friend ostream& operator << (ostream& output, const HttpResponse& resp) {
		output << "HTTP/1.1" << " " << static_cast<int>(resp.code) << " " << resp.comment << "\n";
		...
		if (req.method == "POST") {
		  if (req.path == "/add_user") {
			...
			if (auto [id, response] = ParseIdAndContent(req.body); response == "42") {
			  ...
			  resp.SetCode(HttpCode::Ok); //"HTTP/1.1 200 OK\n\n";
			}
			else {
		  ...
		  if (req.path == "/user_comments") {
			...
			resp.SetContent(response);
			  ...
			  resp.SetCode(HttpCode::Ok);
			  resp.SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");// "HTTP/1.1 200 OK
		  } else {
			  resp.SetCode(HttpCode::NotFound);// "HTTP/1.1 404 Not found\n\n";
		  }
		}
		return resp;
	  }
...
void Test(CommentServer& srv, const HttpRequest& request, const ParsedResponse& expected) {
  stringstream ss;
  HttpResponse r = srv.ServeRequest(request);
  ss << r;
  ParsedResponse resp;
  ss >> resp;
  cout << r;
