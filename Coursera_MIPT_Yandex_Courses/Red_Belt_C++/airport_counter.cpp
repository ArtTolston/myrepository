...
public:
	...
	AirportCounter() {
		int n = static_cast<int32_t>(TAirport::Last_);
		for (int i = 0; i < n; ++i) {
			schedule[i].second = 0;
			schedule[i].first = static_cast<TAirport>(i);
		}
	}
	...
	AirportCounter(TIterator begin, TIterator end) {
		...
		while (begin != end) {
			++schedule[static_cast<int32_t>(*begin)].second;
			++begin;
		}
	}
	...
	size_t Get(TAirport airport) const {
		return schedule.at(static_cast<int32_t>(airport)).second;
	}

	// добавить данный элемент
	void Insert(TAirport airport) {
		++schedule[static_cast<int32_t>(airport)].second;
	}

	// удалить одно вхождение данного элемента
	void EraseOne(TAirport airport) {
		if(schedule[static_cast<int32_t>(airport)].second != 0)
			--schedule[static_cast<int32_t>(airport)].second;
	}  //erase one element from airport(one departure)

	// удалить все вхождения данного элемента
	void EraseAll(TAirport airport) {
		schedule[static_cast<int32_t>(airport)].second = 0;
	} // erase all departures of particular airport

	using Item = pair<TAirport, size_t>;
	using Items = array<Item, static_cast<int32_t>(TAirport::Last_)>;  // container of pairs
...
private:
	array<Item, static_cast<int32_t>(TAirport::Last_)> schedule;
};
